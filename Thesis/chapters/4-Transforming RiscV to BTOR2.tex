...
implemented opcodes. This is shown in \figref{fig:constants}.
...
algorithm, which is shown in \algoref{alg:progressiveconsts}.
...
\algoref{alg:generateconstantsfromstate} and comes \textbf{before}
\figref{fig:states}, while the actual initialization is shown in
\algoref{alg:initstate} and comes \textbf{after} \figref{fig:states}.
...
model, this is shown in \figref{fig:extractimmbytype}. This is the
same method I used to get the immediate in the RISC-V simulation
\secref{sec:runInstr} Three points are noteworthy:\\ First, some
...
\secref{sec:riscvIsa}, the immediate is always sign-extended. This is
...
$funct7$. When compared to the decision tree from \figref{fig:tree},
...
instructions as divided in \tabref{tab:rv64i-instructions}. These
examples are found in \figref{fig:valueexample}, specifically for
\begin{itemize}
    \item \texttt{AUIPC} in \figref{auipc},
    \item \texttt{JALR} in \figref{jalr},
    \item \texttt{BEQ} in \figref{beq},
    \item \texttt{LHU} in \figref{lhu},
    \item \texttt{SD} in \figref{sd},
    \item \texttt{ANDI} in \figref{andi},
    \item \texttt{SLLIW} in \figref{slliw},
    \item \texttt{SLT} in \figref{slt}, and
    \item \texttt{SUBW} in \figref{subw}.
\end{itemize}
...
Other properties can be defined, such as terminating on a specific pc
value or when a register reaches a specified value. An example for
this can be the addition of \figref{fig:badvalue} to the model. The
...
\input{figures/4-Transform/badvalue.tex}
