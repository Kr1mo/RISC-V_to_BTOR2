...
address space can be altered with. \todo{benchmark auswirkungen von memory
    size} \input{figures/4-Transform/constants.tex}
...
The immediate on the other hand must be first constructed from its subfields, which can be referenced in \figref{fig:rv64i_formats}.
In the BTOR2 model this looks like in \figref{fig:extractimmbytype}.
\todo{Reference to same method in riscvsim}
...
Second, as described in \secref{sec:riscvIsa} the immediate is always sign-extended. To archive this we make arithmetic right shifts, which do sign extension for us and with this pull our highest immediate bit to its correct place.
...
This is done in \figref{fig:findingImm}, where first I defined Bools wich check all opcodes that are neither R-type nor I-type.
...
So I defined a check \textsl{is\textcolor{Green}{Instruction}} for each instruction. As this is quite repetetive, \algoref{alg:commanddetection}
describes a generalised approach to reach these Bools. An example for each instruction subgroup in \algoref{alg:commanddetection} can be found in \figref{fig:detectionexample}.
