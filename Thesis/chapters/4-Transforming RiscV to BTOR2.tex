...
\todo{Einfache benchmarks in seinem btor2 modell schreiben und
    vergleichen?}
...
\todo{benchmark auswirkungen von memory size}
...
Tis is straightforward as shown in \figref{fig:states}. I also
...
mention them again. \input{figures/4-Transform/staterep.tex}
...
BTOR2 model must be handled. I decided to just initialiase the
...
behaivour. Everything not mentioned in the state file will be
zero-initialised. At last these constants must be used to initialise
the state. For the registers this is straight forward, for the memory
we must first write all memory adresses into a placeholder array wich
then we can use to initialise the real memory. Due to constraints in
BTOR2, these constants have to be defined \textbf{before} the states,
but initialisation with the values must happen after the states. This
means that this initialisation process \textbf{wrappes around} the
...
This is shown in \figref{fig:extractNOimm}.
...
\figref{fig:findingImm}, where first I defined Bools wich check all
...
this is quite repetetive, \algoref{alg:commanddetection} describes a
...
\figref{fig:nextpc}. Only the behaivour if no 'if' catches is
...
\figref{alg:nextrd}. Whith exeption of x0 this is the same for all
